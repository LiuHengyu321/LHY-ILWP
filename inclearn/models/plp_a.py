...
class PLP_A(IncrementalLearner):
    ...
    def _training_step(
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            if self._eval_every_x_epochs and epoch != 0 and epoch % self._eval_every_x_epochs == 0:
                ...
                fine_ypred, fine_ytrue, coarse_ypred, coarse_ytrue = self._eval_task(val_loader)

                fine_acc = 100 * round((fine_ypred == fine_ytrue).sum() / len(fine_ytrue), 3)
                logger.info("Val fine accuracy: {}".format(fine_acc))
                coarse_acc = 100 * round((coarse_ypred == coarse_ytrue).sum() / len(coarse_ytrue), 3)
                logger.info("Val coarse accuracy: {}".format(coarse_acc))

                # ytrue = torch.tensor(ytrue)
                # logits = torch.tensor(logits)
                # onehot = torch.zeros(ytrue.shape[0], self._n_classes)
                # onehot.scatter_(dim=1, index=ytrue.long().view(-1, 1), value=1.)
    ...
    def _forward_loss(
        ...
        **kwargs
        ...
        # loss = coarse_loss * 0.5 + fine_loss * 0.5
        loss = fine_loss
