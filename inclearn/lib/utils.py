...
def extract_features(model, loader):
    fine_targets, coarse_targets, features = [], [], []
    ...
    for input_dict in loader:
        ...
        _fine_targets, _coarse_targets = input_dict["fine_targets"], input_dict["coarse_targets"]

        _fine_targets = _fine_targets.numpy()
        _coarse_targets = _coarse_targets.numpy()

        _features = model.extract(inputs.to(model.device)).detach().cpu().numpy()

        features.append(_features)
        fine_targets.append(_fine_targets)
        coarse_targets.append(_coarse_targets)

    model.train(state)

    return np.concatenate(features), np.concatenate(fine_targets), np.concatenate(coarse_targets)


def extract_preds(model, loader):
    fine_targets, coarse_targets, fine_features, coarse_features = [], [], [], []
    ...
    for input_dict in loader:
        ...
        _fine_features, _coarse_features = model.extract_preds(inputs.to(model.device))
        _fine_features, _coarse_features = _fine_features.detach().cpu().numpy(), _coarse_features.detach().cpu().numpy()
        fine_features.append(_fine_features)
        coarse_features.append(_coarse_features)
        fine_targets.append(_fine_targets)
        coarse_targets.append(_coarse_targets)

    model.train(state)

    return np.concatenate(fine_features), np.concatenate(coarse_features), np.concatenate(fine_targets), np.concatenate(coarse_targets)


def compute_centroids(model, loader):
    features, fine_targets, coarse_targets = extract_features(model, loader)

    centroids_features, centroids_targets = [], []
    for t in np.unique(fine_targets):
        indexes = np.where(fine_targets == t)[0]
...
def add_new_weights(network, weight_generation, current_nb_classes, task_size, inc_dataset):
    ...
    if weight_generation["type"] == "imprinted":
        ...
        for class_index in range(current_nb_classes, current_nb_classes + task_size):
            ...
            features, _, _ = extract_features(network, loader)
