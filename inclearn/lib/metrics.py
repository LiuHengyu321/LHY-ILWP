...
class MetricLogger:
    ...
    def log_task(self, fine_ypreds, fine_ytrue, coarse_ypreds, coarse_ytrue, task_size, zeroshot=False):
        self.metrics["fine_accuracy"].append(
            accuracy_per_task(fine_ypreds, fine_ytrue, task_size=10, topk=1)
        )  # FIXME various task size
        self.metrics["fine_accuracy_top5"].append(
            accuracy_per_task(fine_ypreds, fine_ytrue, task_size=None, topk=5)
        )
        self.metrics["coarse_accuracy"].append(
            accuracy_per_task(coarse_ypreds, coarse_ytrue, task_size=10, topk=1)
        )
        self.metrics["accuracy_per_class"].append(
            accuracy_per_task(fine_ypreds, fine_ytrue, task_size=1, topk=1)
        )
        self.metrics["coarse_accuracy_per_class"].append(
            accuracy_per_task(coarse_ypreds, coarse_ytrue, task_size=1, topk=1)
        )
        self.metrics["incremental_accuracy"].append(incremental_accuracy(self.metrics["fine_accuracy"]))
        self.metrics["incremental_accuracy_top5"].append(
            incremental_accuracy(self.metrics["fine_accuracy_top5"])
        )
        self.metrics["forgetting"].append(forgetting(self.metrics["fine_accuracy"]))
        ...
        if zeroshot:
            seen_classes_indexes = np.where(fine_ytrue < sum(self.increments[:self._task_counter + 1])
                                           )[0]
            self.metrics["seen_classes_accuracy"].append(
                accuracy(fine_ypreds[seen_classes_indexes], fine_ytrue[seen_classes_indexes])
            )
            unseen_classes_indexes = np.where(
                fine_ytrue >= sum(self.increments[:self._task_counter + 1])
            )[0]
            self.metrics["unseen_classes_accuracy"].append(
                accuracy(fine_ypreds[unseen_classes_indexes], fine_ytrue[unseen_classes_indexes])
            )

        if self._task_counter > 0:
            self.metrics["old_accuracy"].append(old_accuracy(fine_ypreds, fine_ytrue, task_size))
            self.metrics["new_accuracy"].append(new_accuracy(fine_ypreds, fine_ytrue, task_size))
    ...
    def last_results(self):
        results = {
            "task_id": len(self.metrics["fine_accuracy"]) - 1,
            "fine_accuracy": self.metrics["fine_accuracy"][-1],
            "coarse_accuracy": self.metrics["coarse_accuracy"][-1],
            "incremental_accuracy": self.metrics["incremental_accuracy"][-1],
            "fine_accuracy_top5": self.metrics["fine_accuracy_top5"][-1],
            ...
            "coarse_accuracy_per_class": self.metrics["coarse_accuracy_per_class"][-1],
