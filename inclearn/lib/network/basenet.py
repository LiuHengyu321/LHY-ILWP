...
from .classifiers import (Classifier, CosineClassifier, DomainClassifier, MCCosineClassifier, PLP_Classifier, PLP_A_Classifier)
...
class BasicNet(nn.Module):
    ...
    def __init__(
        ...
        elif classifier_kwargs["type"] == "plp_fc":
            self.classifier = PLP_Classifier(
                self.convnet.out_dim, device=device, **classifier_kwargs
            )
        elif classifier_kwargs["type"] == "plp_fc_a":
            self.classifier = PLP_A_Classifier(
                self.convnet.out_dim, device=device, **classifier_kwargs
            )
        else:
    ...
    def add_classes(self, fine_classes_inc, coarse_classes_inc, H):
        if self.classifier.classifier_type == 'plp_fc':
            self.classifier.add_classes(fine_classes_inc, coarse_classes_inc, H)
        elif self.classifier.classifier_type == 'plp_fc_a':
            self.classifier.add_classes(fine_classes_inc, coarse_classes_inc, H)
        elif self.classifier.classifier_type == 'fc':
            self.classifier.add_classes(fine_classes_inc)
    ...
    def extract_preds(self, x):
        outputs = self.convnet(x)
        outputs = self.classifier(outputs["features"])
        return outputs["fine_logits"], outputs['coarse_logits']
        # if self.extract_no_act:
