...
class Classifier(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.n_classes = 0
        self.coarse_class_num = 0
        self.fine_class_num = 0
...
class PLP_Classifier(nn.Module):
    classifier_type = "plp_fc"
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.normalize = normalize
        self._weights = nn.ParameterList([])
        self._coarse_weights = nn.ParameterList([])
        self._bias = nn.ParameterList([]) if self.use_bias else None
    ...
    @property
    def coarse_weights(self):
        return torch.cat([w for w in self._coarse_weights])

    @property
    def coarse_new_weights(self):
        return self._coarse_weights[-1]

    @property
    def coarse_old_weights(self):
        if len(self._coarse_weights) > 1:
            return self._coarse_weights[:-1]
        return None
    ...
    def forward(self, features):
        ...
        # logits = F.linear(features, weights, bias=self.bias)
        Pc = self.coarse_weights
        Pf = torch.mm(self.H.T, self.coarse_weights) + self.weights

        # Pc = Pc / torch.norm(Pc, dim=-1, keepdim=True)
        # Pf = Pf / torch.norm(Pf, dim=-1, keepdim=True)
        # features = features / torch.norm(features, dim=-1, keepdim=True)
        output_c = torch.mm(features, Pc.T)
        output_f = torch.mm(features, Pf.T)
        # output_c = torch.mm(features, Pc.T)
        # output_p = torch.mm(output_c, self.H)
        # output3 = torch.mul(output_p, output_f)

        return {"fine_logits": output_f, 'coarse_logits': output_c}

    def add_classes(self, fine_classes_inc, coarse_classes_inc, H):
        if self.coarse_class_num == 0:
            coarse_classes_inc += 1
        self._weights.append(nn.Parameter(torch.randn(fine_classes_inc, self.features_dim)))
        self._init(self.init_method, self.new_weights)
        coarse_inc = 0
        for i in range(len(coarse_classes_inc)):
            if coarse_classes_inc[i] not in self.coarse_class_num:
                coarse_inc += 1
                self.coarse_class_num.append(coarse_classes_inc[i])
        if coarse_inc > 0:
            self._coarse_weights.append(nn.Parameter(torch.randn(coarse_inc, self.features_dim)))
            self._init(self.init_method, self.coarse_new_weights)

        self.use_bias = False

        self.fine_class_num += fine_classes_inc

        H1 = H[: len(self.coarse_class_num), : self.fine_class_num]
        self.H = nn.Parameter(torch.tensor(H1, requires_grad=False, dtype=torch.float), requires_grad=False)
...
class PLP_A_Classifier(nn.Module):
    classifier_type = "plp_fc"
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._fine_fc_weights = nn.ParameterList([])
        self._coarse_fc_weights = nn.ParameterList([])
    ...
    # fine_weights for fc
    @property
    def fine_fc_weights(self):
        return torch.cat([w for w in self._fine_fc_weights])

    @property
    def new_fine_fc_weights(self):
        return self._fine_fc_weights[-1]

    @property
    def old_fine_fc_weights(self):
        if len(self._fine_fc_weights) > 1:
            return self._fine_fc_weights[:-1]
        return None

    # coarse weights for fc
    @property
    def coarse_fc_weights(self):
        return torch.cat([w for w in self._coarse_fc_weights])

    @property
    def new_coarse_fc_weights(self):
        return self._coarse_fc_weights[-1]

    @property
    def old_coarse_fc_weights(self):
        if len(self._coarse_fc_weights) > 1:
            return self._coarse_fc_weights[:-1]
        return None
    ...
    def forward(self, features):
        ...
        alpha_c = torch.mm(features, Pc.T)
        alpha_f = torch.mm(features, Pf.T)

        attn_c = torch.mm(alpha_c, Pc)
        attn_f = torch.mm(alpha_f, Pf)

        fine_fc_weights = self.fine_fc_weights
        coarse_fc_weights = self.coarse_fc_weights

        output_f = F.linear(attn_f, fine_fc_weights, bias=None)
        output_c = F.linear(attn_c, coarse_fc_weights, bias=None)
    ...
    def add_classes(self, fine_classes_inc, coarse_classes_inc, H):
        ...
        self._fine_fc_weights.append(nn.Parameter(torch.randn(fine_classes_inc, self.features_dim)))
        self._init(self.init_method, self.new_fine_fc_weights)
        ...
        if coarse_inc > 0:
            ...
            self._coarse_fc_weights.append(nn.Parameter(torch.randn(coarse_inc, self.features_dim)))
            self._init(self.init_method, self.new_coarse_fc_weights)
