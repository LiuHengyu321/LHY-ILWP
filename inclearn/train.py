...
def _train(args, start_date, class_order, run_id):
    ...
    for task_id in range(inc_dataset.n_tasks):
        ...
        fine_ypreds, fine_ytrue, coarse_ypreds, coarse_ytrue = model.eval_task(test_loader)
        metric_logger.log_task(
            fine_ypreds, fine_ytrue, coarse_ypreds, coarse_ytrue, task_size=task_info["increment"], zeroshot=args.get("all_test_classes")
        )
        ...
        if args["dump_predictions"] and args["label"]:
            ...
            with open(
                ...
                pickle.dump((fine_ypreds, fine_ytrue), f)

        if args["label"]:
            logger.info(args["label"])
        logger.info("Avg fine inc acc: {}.".format(metric_logger.last_results["incremental_accuracy"]))
        logger.info("Current fine acc: {}.".format(metric_logger.last_results["fine_accuracy"]))
        logger.info("Current coarse acc: {}.".format(metric_logger.last_results["coarse_accuracy"]))
        logger.info(
            "Avg fine inc acc top5: {}.".format(metric_logger.last_results["incremental_accuracy_top5"])
        )
        logger.info("Current fine acc top5: {}.".format(metric_logger.last_results["fine_accuracy_top5"]))

        logger.info("Accuracy per fine class:{}.".format(metric_logger.last_results["accuracy_per_class"]))
        logger.info("Accuracy per coarse class:{}.".format(metric_logger.last_results["coarse_accuracy_per_class"]))
        ...
        last_acc = results["results"][-1]["fine_accuracy"]["total"]
...
def _train_task(config, model, train_loader, val_loader, test_loader, run_id, task_id, task_info):
    if config["resume"] is not None and os.path.isdir(config["resume"]) \
       and ((config["resume_first"] and task_id == 0) or not config["resume_first"]):
        ...
        model.train()
        model.train_task(train_loader, test_loader)
...
def _set_data_model(config, class_order):
    ...
    config["classes_order"] = inc_dataset.fine_class_order

    model = factory.get_model(config)
    model.inc_dataset = inc_dataset
    model.H = inc_dataset.H
